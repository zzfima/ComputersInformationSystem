...
const string ServiceName = "MQ Alive IP Gather Producer Service";
...
_builder.Services.AddSingleton<IMessageQueueProducer, RabbitMQProducer>(m =>
{
    ...
    mqService?.Initialize(
        _configuration.MQHostName,
        _configuration.MQUserName,
        _configuration.MQPassword,
        _configuration.MQAliveIPGatherRoutingKey);
    return mqService;
});
